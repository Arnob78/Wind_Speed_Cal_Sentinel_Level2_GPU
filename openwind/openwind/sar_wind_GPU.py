...
class SARWind(Nansat, object):
    ...
    def __init__(self, sar_image, wind_direction='ncep_wind_online',
        ...
        if isinstance(sar_image, str):
            super(SARWind, self).__init__(sar_image, mapper='sentinel1_l1', *args, **kwargs)
    ...
    def _get_aux_wind_from_str(self, aux_wind_source, *args, **kwargs):
        if isinstance(aux_wind_source, str):
            try:
    ...
    def _calculate_wind(self):
        ...
        windspeed = cmod5n_inverse_gpu(self[self.sigma0_bandNo],
    ...
    def save_wind_map_image(self, fileName, scale=None, numArrowsRange=10,
        ...
        # Get projection from Nansat object
        wkt_projection = self.vrt.dataset.GetProjection()
        if wkt_projection:
            crs = ccrs.CRS.from_wkt(wkt_projection)
        else:
            print("Warning: Projection not found in source data. Defaulting to PlateCarree (WGS84).")
        ...
        # Get wind speed data and coordinates
        windspeed = self['windspeed']
        lon_grid, lat_grid = self.get_geolocation_grids()

        # Plot wind speed as pcolormesh
        pcolormeshArgs = {'vmin': 0, 'vmax':20} # Default values from original code
        ax.pcolormesh(lon_grid, lat_grid, windspeed, transform=ccrs.PlateCarree(), **pcolormeshArgs)

        # Calculate Ux and Vx for quiver plot
        winddirection = np.mod(self['winddirection'] + 180, 360)
        # The original code used a fixed model_windspeed for scaling, let's use the actual windspeed
        # For quiver, we need to subsample the data to avoid overcrowding
        numArrowsX = 20 # Number of arrows in X direction
        numArrowsY = 20 # Number of arrows in Y direction

        # Subsample the data for quiver plot
        stepX = max(1, int(lon_grid.shape[1] / numArrowsX))
        stepY = max(1, int(lat_grid.shape[0] / numArrowsY))

        lon_sub = lon_grid[::stepY, ::stepX]
        lat_sub = lat_grid[::stepY, ::stepX]
        windspeed_sub = windspeed[::stepY, ::stepX]
        winddirection_sub = winddirection[::stepY, ::stepX]

        # Convert wind speed and direction to U and V components
        # Wind direction is 'from' in degrees, 0 from North, 90 from East
        # Convert to 'to' direction for standard vector plotting
        angle_rad = np.radians(winddirection_sub + 180) # Convert to 'to' direction and then to radians
        Ux = windspeed_sub * np.sin(angle_rad)
        Vx = windspeed_sub * np.cos(angle_rad)

        # Plot quiver
        ax.quiver(lon_sub, lat_sub, Ux, Vx, transform=ccrs.PlateCarree(),
                  scale=500, # Adjust scale as needed
                  headwidth=5, headlength=7, headaxislength=6, minlength=0.5,
                  width=0.002, color='black')

        # Add colorbar
        cbar = fig.colorbar(ax.collections[0], ax=ax, orientation='horizontal', pad=0.05, shrink=0.8)
...
if __name__ == '__main__':
    ...
    if args.netCDF is not None:
        print('Saving output to GeoTIFF file: ' + args.netCDF)
        sw.export(args.netCDF.replace('.nc', '.tif'), bands=[sw.get_band_number('windspeed')], driver='GTiff')
