...
#  !  ANGLES
    ...
    S = A2*V
    S_vec = S.copy() 
    SlS0 = [S_vec0 V = V + step V[ind] = V[ind] - 2*step step = step/2 #mdict={'s0obs':sigma0_obs,'s0calc':sigma0_calc} #from scipy.io import savemat #savemat('s0test',mdict) return V
    S_vec[SlS0]=S0[SlS0]
    A3=1./(1.+exp(-S_vec))
        SlS0 = (S_vec < S0)
    S_vec[SlS0] = S0[SlS0]
